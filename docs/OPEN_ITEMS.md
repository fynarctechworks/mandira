...
|---|---|---|---|
| GIT-01 | **Push credentials for `fynarctechworks/mandira`.** The stored credential is for `stimuliIQ`, which has no write access. Either grant that account write access, or clear `git:https://github.com` from Windows Credential Manager and re-auth. **Now the single blocker on finishing M1** — B-025 built the deploy pipeline, backups and runbook, and none of it can execute. | Every commit so far (47 on the local branches as of 2026-09-13, none pushed); all CI; the entire B-025 deploy | CI has never executed. Two workflow jobs and ~10 steps are unverified, including `supabase/setup-cli` on a clean runner and the generated-types staleness diff. |
...
| ACCT-03 | *With the key set, the Ops location picker shows the pin on a static map (D-173).* **MapTiler key** (`NEXT_PUBLIC_MAPTILER_KEY`, free tier). The coordinate picker works without it (search + decimal degrees), but map confirmation of pins needs a tile source. | Map confirmation in the editors; **MAPS-02 only** — MAPS-01 and MAPS-03 shipped without it (D-105) | Before MAPS-02 |
...
| ACCT-06 | *The adapter is built and wired into both apps (D-173); setting the DSN is all that remains.* **Sentry account + DSN** (`SENTRY_DSN`, free tier: 5k errors/month per TRD §9). **Nothing is blocked** — B-024 built the reporting seam and wired it into every route and both error boundaries, with a structured-console transport that Vercel collects as log lines (D-117). Installing `@sentry/nextjs` is one adapter once a DSN exists; it was deliberately NOT installed early, since a heavy inert dependency spends the ≤180 kB route budget on nothing. | Error alerting only | Before B-025's deploy |
...
| OPEN-009 | **NARROWED, no longer blocking.** The half that blocked B-015 is resolved: `0015` exposes accessibility on places/experiences/routes and stops on routes, along the recommended lines (D-079, D-080). **`live_feed_readings` opened in B-031** via `v_published_live_conditions` — provider and timestamp always present, the feed's `config` (which can hold credentials) deliberately absent, and the base tables still closed. Still deliberately closed, because nothing reads them yet: `circuits` (M5) and `destination_links`. Reopen when a feature needs one — opening a surface before something reads it is how nobody notices it was opened too far. | B-031 only | With B-031 |
...
| OPEN-014 | **RESOLVED 2026-09-13 (D-164) — built without a claims table.** Extracted claims are kept in `ai_extractions`, reach review as `change_candidates.new_value`, and `record_extraction` (0034) opens a conflict when a T3-or-better source disagrees with a differently-sourced reviewed value. It runs as soon as ACCT-04 supplies a model key. The original analysis follows. ~~**Automatic conflict detection cannot be built on the current schema — a decision is needed.**~~ PRD-OPS-SRC-005 wants a Conflict opened when two sources of tier ≤T3 give different values for a field, and `conflicts.values` is `{source_id, tier, value, captured_at}` (TRD §4.3). But `trust_records` is `unique (entity_table, entity_id, field_name)`: it holds the ONE source a field is verified against, with nowhere for a second source's competing claim. The only producer of per-source claims is AI extraction, which needs **ACCT-04**. **Recommendation:** ship the Conflicts queue and its resolution (PRD-OPS-WF-003) with a manual raise — which is what a verifier does today when they read two sources — and wire auto-detection to extraction when the key lands. The alternative is a claims table the TRD does not describe, which is a schema decision for the founder rather than for me (D-145). | PRD-OPS-SRC-005 auto-detection | Before the second destination is ingested |
| TEST-02 | **Resolved 2026-09-13 (D-173):** `apps/ops/lib/ingestion.test.ts` drives the real runner with a stub capture and an in-memory database — capture stored, candidate opened for the vanished excerpt, unchanged page recorded without a copy, a failed fetch written onto the job. Originally: **The ingestion runner's fetch-to-detection seam has no end-to-end test.** The capture provider refuses loopback and private addresses — correctly, since the feature fetches operator-typed URLs — so no E2E can point it at a fixture server, and relaxing the guard for a test is forbidden (CLAUDE.md §5). Both sides are covered (35 vitest, 19 pgTAP); the three lines joining them are not. The provider is injectable, so the fix is a runner test with a stub capture and a local database. No founder decision needed. | Confidence in B-029 | Before the launch destination is monitored for real |
| TEST-01 | **The traveler E2E suite runs out of hourly write budget as it grows.** `journeys_write` is 120/hour per USER (TRD §6.2) and one account was standing in for the whole suite. It went red at 130 when the Record spec landed and again at 136 when the knowledge-check tests did. Three accounts now: `web-traveler-b` carries Record and Changes, and `web-traveler-c` (added 2026-09-13) carries add-to-journey, reorder and profile, the three new write-heavy specs that would have pushed the first account over again. **When the next spec pushes one over, the answer is another traveler — never a larger limit** (CLAUDE.md §5). No founder decision needed; recorded so the next 429 is recognised in seconds. | Suite reliability | Before the next write-heavy traveler spec |
...
| DEV-01 | **Resolved 2026-09-13 by D-182** (not adopted, by decision). Originally: **TanStack Query not adopted**, though FRONTEND_ARCHITECTURE §11.1 specifies it with a sync-storage persister. Every traveler read page is a server component, so adopting it means converting the read path to client components — a large rewrite whose only offline gain is a second cache in front of Dexie. §11.2's `useOfflineFirst` is implemented directly against Dexie instead, which satisfies TRD-ARCH-005's "Dexie-first + revalidate" with one cache. Revisit only if a screen needs it for something other than persistence. | Nothing today | Review at M2 |
| OPEN-013 | **Can a guest report a change?** `user_reports` MODELS a reporter with no account — `user_id` is nullable and `reporter_hash` exists for exactly that — but no anon policy or grant was ever written, so guest reporting does not work. PRD F14 does not settle it. **The case for allowing it:** the person who notices a gate is shut is standing in front of it, and requiring an account first is how that observation is lost. **The case against:** it widens the only anon write surface besides analytics, on a table holding free text, so spam becomes possible (mitigated by the 5/hour `reports_create` limit and the device cookie). B-028 followed the policy and required sign-in rather than deciding this in passing. | Guest reporting only; signed-in reporting works | Before the pilot |
| OPEN-012 | **RESOLVED 2026-09-13 (D-160).** ~~`POST /api/journeys` is not transactional.~~ The route now saves through `create_journey` (0031), one SQL function and one transaction, with the owner taken from the session; pgTAP `0032` proves a refused item leaves no half-saved journey. | — | — |
...
| MON-01 | **Provider usage counters for the 70 % free-quota alert (TRD §13).** Record calls per provider per day (Gemini extraction and describe-your-journey, OpenRouteService routing, Resend email, MapTiler tiles), with each free tier's limit as configuration, so the Ops dashboard can show usage and the daily alert (0046) can include any provider at or above 70 %. Gemini exposes no usage endpoint on the free tier, so our own counters are the only source there. The queue-age half of the alert is built (D-201). | The quota half of TRD §13's cost controls | Before traffic approaches a free tier; cheapest to add before launch |
