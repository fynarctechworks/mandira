...
| ID | Resolution |
|---|---|
| MON-01 | **Resolved (D-206, migration 0050).** `provider_quotas` holds each free tier's limit per day or month as data; `provider_usage` counts OpenRouteService legs and Resend sends per UTC day, and Gemini is counted from `ai_calls`. O01 shows used against quota, and the daily Ops email (0046) now names any provider at 70 % or more. Not counted: MapTiler tiles (loaded in the browser) and sign-in emails Supabase Auth sends through Resend SMTP — both noted on the dashboard and in the quota rows. |
