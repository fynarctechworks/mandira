...
| TEST-01 | **The traveler E2E suite runs out of hourly write budget as it grows.** `journeys_write` is 120/hour per USER (TRD §6.2) and one account was standing in for the whole suite. It went red at 130 when the Record spec landed and again at 136 when the knowledge-check tests did. Two accounts now (`web-traveler` and `web-traveler-b`, carrying the two write-heaviest specs), sitting at 105 and 50. **When a third spec pushes the first over again, the answer is a third traveler — never a larger limit** (CLAUDE.md §5). No founder decision needed; recorded so the next 429 is recognised in seconds. | Suite reliability | Before the next write-heavy traveler spec |
