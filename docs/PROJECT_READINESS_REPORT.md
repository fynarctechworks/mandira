...
| OPEN-009 | **Traveler access to accessibility, route-stop, circuit and destination-link data.** TRD §4.4 lists 9 published views; none covers `accessibility_records`, `route_places`, `circuits`, `destination_links` or `live_feed_readings`, yet PRD-DISC-003 requires accessibility icons on experience cards and F10 requires live-feed labels. They are deny-by-default after B-006 (D-033). Recommendation: extend `v_published_places`/`v_published_experiences` with an accessibility jsonb rather than adding views. | B-015 (discovery), B-031 (live feeds) | Before B-014 |
