...
| Ops core | Ops shell + editors | PRD-OPS-CNT-001, PRD-KNOW-001 | IN_PROGRESS | AUTH-05 | 2026-08-25 | B-008 shell + B-009 editors done: destinations (O02) and places (O03) with i18n tabs, geocoding-backed coordinate picker, and the opening-schedule builder. `opsAction()` wrapper (D-043); Nominatim `GeocodingProvider` with the policy rate limit. 119 vitest + 15 Playwright (real create/read-back). Plans: docs/plans/OPS-SHELL.md, docs/plans/OPS-EDIT-01-02.md. Next: B-010 remaining editors. |
